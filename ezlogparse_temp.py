...
class parse_ezlog(object):
	...
	def search(self, keyword):
		...
		#self.filtered_items.append(["", "", "", " [19/Apr/2018:00:00:00", "+0000]", "", "", "", "", ""])
	...
	def finale2(self):
		self.unique_items = zip(self.ip, self.request)
		seen = set()
		for i, ele in enumerate(self.unique_items, 1):
			if ele not in seen:
...
def generate_statistics(items, timewindow, flag):
	...
	#finaltime = items.unixtime[len(items.unixtime)]
	...
	#string += 'Final timestamp: {0} [{1}]\n'.format(items.unixtime[len(items.unixtime)], len(items.unixtime))
